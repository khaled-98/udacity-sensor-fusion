...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # otherwise False.
        ############
        padded_x = np.ones((4, 1))
        padded_x[0:3] = x[0:3]
        transformed_x = self.veh_to_sens * padded_x

        if transformed_x[0] > 0: 
            angle = np.arctan(transformed_x[1] / transformed_x[0]) # calc angle between object and x-axis
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            hx = np.zeros((2, 1))
            if transformed_x[0] == 0:
                raise ValueError('jacobian is not defined')
            else:
                hx[0, 0] = self.c_i - self.f_i * transformed_x[1] / transformed_x[0]
                hx[1, 0] = self.c_j - self.f_j * transformed_x[2] / transformed_x[0]
